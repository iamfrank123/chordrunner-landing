...
class GameScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    if (this._levelIndex === 'custom' && this._customOptions) {
      ...
      const naturalRoots = ['C', 'D', 'E', 'F', 'G', 'A', 'B'];
      const allRoots = ['C', 'C#', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B'];
      this._survivalRoots = this._survivalOpts?.roots === 'all' ? 'all' : 'natural';
      const rootsToUse = this._survivalRoots === 'all' ? allRoots : naturalRoots;

      const types = ['', 'm', 'dim', 'aug', 'sus2', 'sus4', 'maj7', 'm7', '7', 'm7b5', 'maj9', 'm9', '9'];
      ...
      this._levelData = { id: 'adaptive_survival', name: 'Adaptive Survival', chords: survivalChords };
  ...
  create() {
    ...
    // Calculate global scaling ratio relative to an 800px standard height.
    this._ratio = H / 800;
    this._gravity = CONFIG.GRAVITY * this._ratio;
    this._jumpForce = CONFIG.JUMP_FORCE * this._ratio;
    this._obsSpeed = CONFIG.BASE_OBSTACLE_SPEED * this._ratio;
    
    // Convert hardcoded layout positions to fluid layout
    this._triggerX = W * 0.22;
    this._playerX = W * 0.12;
    ...
    if (this._isSurvival) {
      this._adaptiveDifficulty = new AdaptiveDifficulty();
      const unlim = this._survivalOpts.unlimited;
    ...
    this.input.on('pointerdown', () => {
      ...
      if (MidiManager.isVirtual) {
        // Modalità tastiera virtuale: CLICK = SALTO (supera l'ostacolo)
        if (GameStateManager.is(GAME_STATE.RUNNING)) {
          if (this._activeObstacle) {
            this._onCorrectChord(); // Simula l'accordo corretto e salta
          } else {
            this._doJump(); // Salto a vuoto
          }
        }
      } else {
    ...
    // DEBUG / CHEAT HOTKEY K: Auto-play corretto accordo del mostro / ostacolo
    this.input.keyboard?.on('keydown-K', () => {
      if (!this._gameActive) return;
      if (GameStateManager.is(GAME_STATE.COMBAT) && this._combatManager && this._combatManager._currentChord) {
        this._combatManager.onChordInput([this._combatManager._currentChord], ['C', 'E', 'G'], true);
      } else if (GameStateManager.is(GAME_STATE.RUNNING) && this._activeObstacle && this._requiredChord) {
        this._onInput([this._requiredChord], ['C', 'E', 'G'], true);
      }
    });
  ...
  _createGround(W, H) {
    const T = 32;
    this._groundY = H - (T * 1.5 * this._ratio) - (40 * this._ratio);
    ...
    const gb = this._groundBody.create(W / 2, this._groundY + 24 * this._ratio, null);
    gb.setVisible(false); gb.displayWidth = W; gb.displayHeight = 48 * this._ratio; gb.refreshBody();
  }

  _createPlayer(W, H) {
    this._player = this.physics.add.sprite(this._playerX, this._groundY - 2, 'player');
    this._playerBaseScale = 2.38 * this._ratio;
    this._player.setOrigin(0.5, 1).setDepth(9).setScale(this._playerBaseScale);
    this._player.setCollideWorldBounds(true);
    this._player.body.setGravityY(this._gravity);
  ...
  _doJump() {
    if (this._player.body.blocked.down || this._player.y >= this._groundY - 2) {
      ...
      this._player.body.setVelocityY(this._jumpForce);
      const bs = this._playerBaseScale;
      this.tweens.add({ targets: this._player, scaleY: bs * 0.65, scaleX: bs * 1.15, duration: 90, yoyo: true });
    }
  }
  ...
  ───────────────────────────────────────── */
  _drawTriggerLine() {
    const g = this._triggerLine, x = this._triggerX, H = this._groundY;
    g.clear();
    g.lineStyle(2, 0xffffff, 0.15); g.beginPath(); g.moveTo(x, 0); g.lineTo(x, H); g.strokePath();
    g.fillStyle(0xffffff, 0.05); g.fillRect(x - 30 * this._ratio, H - 4, 60 * this._ratio, 4);
    
    if (!this._playHereLabel) {
      this._playHereLabel = this.add.text(x, 80 * this._ratio, '🎵 PLAY HERE', {
        fontFamily: 'Outfit, sans-serif', fontSize: '13px', fontStyle: 'bold',
        color: '#a29bfe', stroke: '#0a0a1a', strokeThickness: 3
      }).setOrigin(0.5, 0).setDepth(6);
    } else {
      this._playHereLabel.setX(x);
    }
  }
  ...
  _spawnObstacle() {
    let chord;
    if (this._isSurvival) {
      chord = this._adaptiveDifficulty.getChord(this._survivalRoots);
    } else {
      chord = randomChord(this._chordPool);
    ...
    obs.setOrigin(0.5, 1).setDepth(7).setScale(1.8 * this._ratio);
    obs.body.setAllowGravity(false);
    obs.body.setVelocityX(-this._obsSpeed);
  ...
  _updateObstacles() {
    ...
    this._obstacles.getChildren().forEach(obs => {
      ...
      if (obs.state === 'incoming' && obs.x <= trigX + 60 * this._ratio) {
  ...
  _onCorrectChord() {
    ...
    if (this._isSurvival && this._adaptiveDifficulty) {
      this._adaptiveDifficulty.recordHit(this._requiredChord, true);
    }
  ...
  _onWrongChord() {
    ...
    if (this._isSurvival && this._adaptiveDifficulty) {
      this._adaptiveDifficulty.recordHit(this._requiredChord, false);
    }
  ...
  _triggerCombat() {
    ...
    const adaptivePct = this._adaptiveDifficulty ? this._adaptiveDifficulty.getPercentages() : null;
    const monster = MonsterSystem.spawnMonster(GameStateManager.getDiffScore(), adaptivePct);
