...
class CombatManager {
  ...
  _buildMonsterVisual() {
    ...
    // Mantiene proporzioni tramite il _ratio calcolato nella GameScene
    this._ratio = scene._ratio || (H / 800);
    this._baseScale = def.size * this._ratio;

    // Sprite mostro posizionato al 72% della larghezza dinamica
  ...
  _animateEntrance(onDone) {
    ...
    if (def.entranceAnim === 'boss') {
      this._monsterSprite.setScale(this._baseScale * 2);
      scene.tweens.add({
  ...
  _createTargetVisual() {
    ...
    // Posizione random tra la linea di azione (PLAY HERE) e il mostro, limitando altezza per stare a schermo
    this._targetX = Phaser.Math.Between(scene._triggerX + 40, monsterX - 160 * scene._ratio);
    this._targetY = scene._groundY - Phaser.Math.Between(80 * scene._ratio, 200 * scene._ratio);
